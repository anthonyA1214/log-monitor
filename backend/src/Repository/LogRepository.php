...
final class LogRepository
{
    ...
    public function findManualLogs(): array
    {
        $sql  = "SELECT * FROM log_files WHERE source = 'manual' ORDER BY file_modified_at DESC";
        $stmt = $this->pdo->prepare($sql);
        $stmt->execute();
    ...
    {
        $sql = 'SELECT id, title, file_name, file_path, file_modified_at FROM log_files WHERE title = :title';
        ...
        \usort(
            $rows,
            static function ($a, $b) {
                return \strcmp(self::extractDateFromFileName($b['file_name']), self::extractDateFromFileName($a['file_name']));
            },
        );
    ...
    {
        $sql  = "SELECT file_name, file_modified_at, file_path FROM log_files WHERE source = 'manual' ORDER BY file_modified_at DESC";
        $stmt = $this->pdo->prepare($sql);
        $stmt->execute();
