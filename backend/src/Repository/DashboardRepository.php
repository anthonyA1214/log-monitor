...
final class DashboardRepository
{
  ...
  {
    $sql = 'SELECT section, slot_number, title FROM dashboard_slots ORDER BY section, slot_number';
  ...
  public function assignSlot(string $section, int $slotNumber, string $title): void
  {
    ...
    try {
      $stmt = $this->pdo->prepare('UPDATE dashboard_slots SET title = NULL WHERE title = :title');
      $stmt->execute([':title' => $title]);

      $stmt = $this->pdo->prepare('UPDATE dashboard_slots SET title = :title WHERE section = :section AND slot_number = :slot_number');
      $stmt->execute([
        'title' => $title,
        'section' => $section,
